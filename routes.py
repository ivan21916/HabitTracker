...
def index():
    ...
    # MongoDB database
    habits_on_date = current_app.db.habits.find({"added": {"$lte": selected_date}}) # $lte is a query meaning less than or equal to, which means today or earlier
    completions = [
        habit["habit"]
...
def add_habit():
    ...
    if request.method == "POST": # Or if request.form
...
@pages.route("/complete", methods=["POST"]) # Same as @app.post("/complete")
def complete():
